...
// =====================================================================
// AURUM 3D TOKEN LOADER
// An original onyx + rose-gold disc-shaped value token.
// Interactions:
//   • Auto-spins continuously (Y axis) by default
//   • Right-click + drag → free 3D rotation (auto-spin pauses)
//   • Left-click + drag  → subtle parallax tilt
//   • Scroll wheel       → zoom camera in/out
//   • Mouse hover (page) → iridescent rose-gold drop-shadow follows cursor
// After ~3s, parent transitions the token to the header (top-left) where
// it keeps spinning. We expose `mode="loader" | "header"` to drive that.
// =====================================================================

const AurumToken3D = ({ mode = 'loader', accentHex = '#E0A899', onReady }) => {
  ...
  useEffect(() => {
    ...
    // Inner inset on each face — concentric rose-gold ring
    ...
    ringFront.rotation.z = 0;
    const ringBack = new T.Mesh(ringGeom, ringMat);
    ringBack.position.z = -thickness / 2 - 0.001;
    ringBack.rotation.y = Math.PI;
    // Reattach: cylinder was rotated so faces are along z after rotation? actually after rotate.x = PI/2, faces point along z.
    tokenGroup.add(ringFront);
    tokenGroup.add(ringBack);

    // Embossed center medallion (inner disc)
    ...
    // Engraved monogram — extrude a custom shape: a stylized "A" formed
    // by a triangle outline + crossbar. Original mark, no IP.
    const buildMonogram = () => {
      ...
      // Triangle frame using extruded shape with hole
    ...
    // Engraved arc text — "AURUM · VALUE TOKEN · MMXXVI" using small
    // cylinders along arc to simulate engraving (no font load needed).
    ...
    // Edge knurling — small rectangular ridges around the rim
    ...
    // Subtle env via a colored hemisphere
    const hemi = new T.HemisphereLight(0xffd9c2, 0x1a1820, 0.5);
    scene.add(hemi);

    // ---- INTERACTION STATE ----
    const s = stateRef.current;
    s.spinSpeed = 0.012;       // base auto-spin
    s.targetZ = 6.2;            // camera z
    s.zNow = 6.2;
    s.tiltX = 0; s.tiltY = 0;   // parallax tilt (left-click drag)
    s.targetTiltX = 0; s.targetTiltY = 0;
    s.freeRot = { x: 0, y: 0 }; // right-click free rotation
    s.useFreeRot = false;
    s.dragging = null;          // 'left' | 'right' | null
    s.last = { x: 0, y: 0 };
    s.autoSpin = true;
    s.modeRef = mode;
    ...
    const onPointerDown = (e) => {
      if (s.modeRef === 'header') return; // disable in header mode
    ...
    const animate = () => {
      ...
      if (s.modeRef === 'header') {
        // header mode: keep spinning, no interaction
  ...
  // keep ref in sync if mode prop changes after mount
...
// ---------------------------------------------------------------------
// AurumLoader — full-screen overlay with the 3D token + iridescent
// cursor-tracking glow + ~3 second auto-dismiss
// ---------------------------------------------------------------------
const AurumLoader = ({ accentHex, accentShimmer, onComplete }) => {
  ...
  // Iridescent halo — three layered radial gradients shifted by hue,
  // tracking mouse position. No real currency, no IP.
  ...
  return (
    ...
    >
      {/* Iridescent halo follows cursor */}
      ...
      <div style={{
        ...
        <div style={{
          fontFamily: '"Instrument Serif", serif', fontSize: 38, letterSpacing: '0.06em',
          fontStyle:'italic', color: '#F2EDE6',
        }}>Aurum</div>
        <div style={{
...
Object.assign(window, { AurumToken3D, AurumLoader });
